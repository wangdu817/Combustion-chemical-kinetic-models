...
FUEL_PATTERNS = [
    ...
    (r"pyrene", "pyrene"),
    # Additional fuels from LLM classification of unknown_fuel papers
    (r"n-nonane", "n_nonane"),
    (r"2-methylhexane", "2_methylhexane"),
    (r"cyclohexadiene", "cyclohexadiene"),
    (r"hexatriene", "hexatriene"),
    (r"JP-?10", "jp10"),
    (r"HMX|octogen|octahydro.tetranitro.tetrazocine", "hmx"),
    (r"TPRF|toluene.primary.reference.fuel", "tprf"),
    (r"pentan-2-ol|2-pentanol", "pentanol"),
    (r"norbornadiene", "norbornadiene"),
    (r"decalin|decahydronaphthalene", "decalin"),
    (r"isopentane|2-methylbutane", "isopentane"),
    (r"phenyl acetate", "phenyl_acetate"),
    (r"ethyl methyl ether", "ethyl_methyl_ether"),
    (r"diethyl carbonate", "diethyl_carbonate"),
    (r"acetic acid", "acetic_acid"),
    (r"n-octanol|1-octanol", "n_octanol"),
    (r"isopropyl nitrate", "isopropyl_nitrate"),
    (r"ethyl levulinate", "ethyl_levulinate"),
    (r"C2HF3Cl2|C2HF5|halon", "halon"),
    (r"diisopropyl ether", "diisopropyl_ether"),
    (r"vinyl acetate", "vinyl_acetate"),
    (r"unsaturated alcohol", "unsaturated_alcohol"),
    (r"di-n-buthylether|di-n-butyl ether", "di_n_butyl_ether"),
    # Ester variants not covered above
    (r"ethyl propanoate|ethyl proprionate", "ethyl_propanoate"),
    (r"methyl propanoate", "methyl_propanoate"),
    (r"ethyl butanoate", "ethyl_butanoate"),
    (r"ethyl pentanoate", "ethyl_pentanoate"),
    (r"methyl decanoate", "methyl_decanoate"),
    (r"propyl acetate", "propyl_acetate"),
    (r"butyl acetate", "butyl_acetate"),
    # Cyclic dienes
    (r"cyclopentadiene", "cyclopentadiene"),
    # Chemical formula notation (in titles: "C3H8", "C 2 H 5 OH", etc.)
    (r"\bC\s*H\s*4\b", "methane"),
    (r"\bC\s*2\s*H\s*6\b", "ethane"),
    (r"\bC\s*3\s*H\s*8\b", "propane"),
    (r"\bC\s*3\s*H\s*6\b", "propene"),
    (r"\bC\s*2\s*H\s*4\b", "ethylene"),
    (r"\bC\s*2\s*H\s*2\b", "acetylene"),
    (r"\bC\s*6\s*H\s*6\b", "benzene"),
    (r"\bC\s*7\s*H\s*8\b", "toluene"),
    (r"\bC\s*8\s*H\s*10\b", "xylene"),
    (r"\bC\s*5\s*H\s*6\b", "cyclopentadiene"),
    (r"\bC\s*2\s*H\s*5\s*O\s*H\b", "ethanol"),
    (r"\bC\s*H\s*3\s*O\s*H\b", "methanol"),
    (r"\bC\s*4\s*H\s*10\b", "n_butane"),
    (r"\bC\s*5\s*H\s*12\b", "pentane"),
    (r"\bC\s*7\s*H\s*16\b", "n_heptane"),
    (r"\bC\s*8\s*H\s*18\b", "n_octane"),
    (r"\bC\s*10\s*H\s*22\b", "n_decane"),
    (r"\bC\s*12\s*H\s*26\b", "n_dodecane"),
    (r"\bN\s*H\s*3\b", "ammonia"),
    (r"\bC\s*4\s*H\s*6\b", "butadiene"),
    # Fuel classes
    (r"\bPRF\b|primary reference fuel", "prf"),
    (r"\bTRF\b|toluene reference fuel", "trf"),
    (r"surrogate fuel|surrogate mixture|gasoline surrogate|diesel surrogate|fuel surrogate", "surrogate"),
    # Oxygenated fuels (broad category)
    (r"\boxygenated (fuel|biofuel|hydrocarbon|compound|species)", "oxygenated"),
    (r"\boxygenates?\b", "oxygenated"),
    # Generic esters (lower priority — place after specific esters)
    (r"\bmethyl ester", "methyl_ester"),
    (r"\bethyl ester", "ethyl_ester"),
    (r"\bC[4-6]\s*ester", "ester"),
    (r"\balkyl ester", "ester"),
    # Additional specific compounds
    (r"tetralin|tetrahydronaphthalene", "tetralin"),
    (r"propanal|propionaldehyde", "propanal"),
    (r"methyl decanoate", "methyl_decanoate"),
]
...
def probe_supplements(max_mmc: int = 12, year: str | None = None, force: bool = False) -> None:
    ensure_dirs()
...
def main() -> None:
    ...
    parser.add_argument("--max-mmc", type=int, default=12)
